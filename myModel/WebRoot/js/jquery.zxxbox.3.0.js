...
(function($) {
	...
	$.extend($.zxxbox, {
		...
		setPosition: function(flag) {
			...
			//主体内容的位置
			//获取当前主体元素的尺寸
			var xh = $.o.out.outerHeight(), xw = $.o.out.outerWidth(),tTemp=0;
			
			//针对iframe 弹出框高度控制   
			if (h != $(window.parent).height() || st != $(window.parent).scrollTop()) {
				h = $(window.parent).height();
				st = $(window.parent).scrollTop();
				tTemp = 320;
			}
			...
			var t = st + (h - xh)/2 - tTemp , l = (w - xw)/2;
