...
function windowResize() {
	...
	logbox.style.height = document.documentElement.clientHeight - other - 100 + 'px';
}
...
function send(text) {
	...
	var param = "type=talk&id=" + id + '&text=' + encodeURIComponent(text);
	JS.AJAX.post('/talkRoom/talk.action', param, function() {
		inputbox.value = '';
	});
}
// 改名动作
function rename() {
	...
	if (!id || !userName || oldName == userName)
		return;
	var param = "type=rename&id=" + id + '&newName=' + encodeURIComponent(userName)
			+ '&oldName=' + encodeURIComponent(oldName);
	setCookie('userName', userName, 365);
	JS.AJAX.post('talkRoom/talk.action', param);
}
